...
interface AccessGateProps {
  onAuthenticated: (key: string, isAdmin: boolean) => void;
}
...
export default function AccessGate({ onAuthenticated }: AccessGateProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Check if it's an admin key by trying admin endpoint
      const { data: adminData } = await supabase.functions.invoke("neon-admin", {
        body: { action: "get_stats", adminKey: key.trim() },
      });

      const isAdmin = adminData && !adminData.error;

      if (!isAdmin) {
        // Validate as regular key
      ...
      localStorage.setItem("neon_admin", isAdmin ? "1" : "0");
      onAuthenticated(key.trim(), isAdmin);
    } catch {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <p className="text-center text-xs text-muted-foreground/50 mt-6">Tool made by TalkNeon</p>
      </div>
    </div>
  );
}
